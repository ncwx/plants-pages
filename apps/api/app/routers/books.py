...
@router.get("/me/library", response_model=List[UserBookOut])
def my_library(user: CurrentUser = Depends(get_current_user)):
    ...
    res = (
        ...
        .select(
            "id,user_id,book_id,status,rating,notes,progress,updated_at,"
            "books:books(id,title,author,cover_url)"
        )
...
def save_to_library(
    ...
    if payload_data.get("progress") is not None and payload_data["progress"] >= 100:
        payload_data["status"] = "finished"
